...
const RegEmp = () => {
  ...
  const isValid = () => {
    ...
    if(!Mobileno.trim() || Mobileno.length > 10) {
      updateError('Mobile Number is Allow Only 10 Digits',setMobileErr);
      return true;
    }
    };
...
{/*<CustomInput
  ...
  <Text style={{fontSize:15,fontWeight:'300',color:'black',padding:5}}>DOB</Text>
  ...
  <Picker
    style={{alignItems:'center',width :'97%',backgroundColor:"white",color:"black"}}
...
<TextInput style={{
   ...
   />
    ...
    <Text style={{fontSize: 25,fontWeight:'bold',color: 'gray', padding: 10}}>Currentcompany</Text>
  <TextInput style={{
        backgroundColor: 'white',
        color:"black",
        width : '100%',
...
{MobileErr ? <Text style={{color:"red",fontWeight:'bold'}}>{MobileErr}</Text>:null}
